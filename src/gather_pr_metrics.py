...
events.append([{'type': 'createAt', 'date': convert_to_datetime(createdAt), 'event_instance':createdAt}])
events.append([{'type': 'commit', 'date': convert_to_datetime(commit['committedDate']), 'event_instance':commit} for commit in commits])
events.append([{'type': 'review', 'date': convert_to_datetime(review['submittedAt']), 'event_instance':review} for review in reviews])
events.append([{'type': 'mergedAt', 'date': convert_to_datetime(mergedAt), 'event_instance':mergedAt}])
events.append([{'type': 'closedAt', 'date': convert_to_datetime(closedAt), 'event_instance':closedAt}])
...
|---|---|
|Time that was spend on the branch before the PR was created|{convert_seconds_to_a_readable_string(time_spend_on_branch_before_pr_created)}|
|Time that was spend on the branch before the PR was merged| {convert_seconds_to_a_readable_string(time_spend_on_branch)} |
|Time to merge after the last review| {convert_seconds_to_a_readable_string(time_to_merge_after_last_review)} |
|Time spend on the PR after creation| {convert_seconds_to_a_readable_string(time_spend_on_pr_after_creation)} |
'''
